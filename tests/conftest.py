...
def fabricate_forecasts():
    ...
    faux_forecasts["AUROC=0"] = [actual_array,
...
def setup_dataframe():
    ...
    for i in np.arange(num_individuals):
        ...
        add_years, add_months = divmod(
            indiv_first_obs_months_from_base_date, 12)
        start_date = base_date + relativedelta(
            years=add_years, months=add_months)

        # Determine separation date for individual
        max_service_months = (max_months_from_base_date -
                              indiv_first_obs_months_from_base_date)
        if mean_service_months > max_service_months:
            actual_service_months = max_service_months
        else:
            actual_service_months = round(np.random.triangular(
                min_service_months, mean_service_months, max_service_months))
        separation_date = start_date + relativedelta(
            months=actual_service_months)

        # Specify variables that are assigned once per individual
        nonmixed_categorical_var = np.random.choice(
            ['A', 'B', 'C', 'D', 'E', 'F'])
        ...
        while date <= separation_date:
            ...
            if ((np.random.binomial(1, 0.8) == 1 and
                 partially_null_numeric_var == 40)):
            ...
            if ((nonneg_normal_numeric_var < 25 or
                 nonneg_normal_numeric_var > 45)):
    ...
    data['_validation'] = pd.Series(np.random.choice(
        [True, False], size=len(data), p=[0.1, 0.9]))
    data['_test'] = (pd.Series(np.random.choice(
        [True, False], size=len(data), p=[0.1, 0.9])) & ~data['_validation'])
    data['_predict_obs'] = (pd.Series(np.random.choice(
        [True, False], size=len(data), p=[0.1, 0.9])) &
                            ~data['_validation'] & ~data['_test'])

    # Add survival duration column
    data = data.sort_values(['SSNSCR', 'FILE_DATE'])
    factorized_time_ids = pd.factorize(data['FILE_DATE'], sort=True)[0]
    data['_duration'] = ((pd.Series(factorized_time_ids)
                          .groupby(data['SSNSCR'])
