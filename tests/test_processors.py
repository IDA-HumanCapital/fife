...
def test_deduplicate_column_values(setup_dataframe):
    ...
    if (('nonneg_uniform_numeric_var' in data.columns and
         'duplicate_nonneg_uniform_numeric_var' in data.columns)):
...
def test_normalize_numeric_feature(setup_dataframe):
    ...
    test_cols = [x for x in df_numeric_vars.columns if
                 x not in ['SSNSCR', 'FILE_DATE']]
    for col in test_cols:
        if col not in ['completely_null_var', 'partially_null_numeric_var']:
            excluded_obs = pd.Series([False] * len(df_numeric_vars[col]))
            normalized_col, numeric_range =\
                processors.normalize_numeric_feature(df_numeric_vars[col],
                                                     excluded_obs)
            if not normalized_col.between(-0.5, 0.5).all():
                errors_list.append(f'Normalized values outside range '
                                   f'[-0.5, 0.5] for column {col}.')
            if not numeric_range[0] == df_numeric_vars[col].min():
                errors_list.append(f'Returned minimum value does not match '
                                   f'actual minimum value for column {col}.')
            if not numeric_range[1] == df_numeric_vars[col].max():
                errors_list.append(f'Returned maximum value does not match '
...
def test_process_numeric_feature(setup_dataframe):
    ...
    for col in test_cols:
        ...
        if ((not scaled_col.between(-0.5, 0.5).all() and
             col != 'completely_null_var')):
            errors_list.append(f'Returned values outside range [-0.5, 0.5] '
                               f'for column {col}.')
    assert not errors_list, ('Errors occurred: \n{}'
                             .format('\n'.join(errors_list)))
...
def test_factorize_categorical_feature(setup_dataframe):
    ...
    test_cols = [x for x in df_categorical_vars.columns if
                 x not in ['SSNSCR', 'FILE_DATE']]
    for col in test_cols:
...
def test_DataProcessor(setup_config, setup_dataframe):
    ...
    assert (data_processor.config == setup_config and
            data_processor.data.equals(setup_dataframe))
...
def test_drop_degenerate_features(setup_config, setup_dataframe):
    ...
    data_processor = processors.DataProcessor(config=setup_config,
...
def test_identify_categorical_features(setup_config, setup_dataframe):
    ...
    """
    reserved_cat_cols = ['_duration', '_event_observed', '_predict_obs',
                         '_test', '_validation']
    reserved_num_cols = ['FILE_DATE']
    non_reserved_cols = [x for x in setup_dataframe.columns if
                         x not in reserved_cat_cols + reserved_num_cols]
    setup_cat_cols = [x for x in non_reserved_cols if
                      'categorical' in x] + ['completely_null_var']
    setup_num_cols = [x for x in non_reserved_cols if 'numeric' in x]
    num_vars_treated_as_cat_vars = []
    for num_col in setup_num_cols:
        if ((setup_dataframe[num_col].nunique() <=
             setup_config['MAX_UNIQUE_NUMERIC_CATS'])):
            num_vars_treated_as_cat_vars.append(num_col)
    setup_cat_cols = (setup_cat_cols + num_vars_treated_as_cat_vars +
                      reserved_cat_cols)
    setup_num_cols = [x for x in setup_num_cols if x not in
                      num_vars_treated_as_cat_vars] + reserved_num_cols
    data_processor = processors.DataProcessor(config=setup_config,
                                              data=setup_dataframe)
...
def test_check_panel_consistency(setup_config, setup_dataframe):
    ...
    data_processor = processors.PanelDataProcessor(config=setup_config,
                                                   data=setup_dataframe)
    data_processor.data = \
        data_processor.data.append(data_processor.data.iloc[[1], :])
...
def test_flag_validation_individuals(setup_config, setup_dataframe):
    ...
    data_processor = processors.PanelDataProcessor(config=setup_config,
                                                   data=setup_dataframe)
    error_tolerance = 0.1
    data_processor.data["validation"] = \
        data_processor.flag_validation_individuals()
