...
def test_binary_encode_feature(setup_config, setup_dataframe):
    ...
    for col in test_cols:
        if df_numeric_vars[col].isnull().all():
            continue
        col_is_categorical = (df_numeric_vars[col].nunique() <=
                              setup_config['MAX_UNIQUE_NUMERIC_CATS'])
        if col_is_categorical:
            binary_cols = tf_modelers.binary_encode_feature(
                df_numeric_vars[col])
            if ((len(binary_cols.columns) !=
                 int(np.log2(df_numeric_vars[col].max())+1))):
                errors_list.append(
                    f'Incorrect number of encoded columns returned for {col}.')
            binary_cols['sum'] = binary_cols.sum(axis=1)
            if ((binary_cols['sum'].max() >
                 int(np.log2(df_numeric_vars[col].max())+1))):
                errors_list.append(
                    f'Returned columns encoded incorrectly for {col}.')
...
def test_split_categorical_features(setup_dataframe):
    ...
    """
    errors_list = []
    categorical_features = (setup_dataframe.select_dtypes(exclude=['number'])
                            .columns.tolist())
    numeric_features = [x for x in setup_dataframe.columns if
                        x not in categorical_features]
    split_features_list = tf_modelers.split_categorical_features(
        setup_dataframe.copy(), categorical_features, numeric_features)
    series_in_split_features_list = [x for x in split_features_list if
                                     isinstance(x, pd.Series)]
    if len(series_in_split_features_list) != len(categorical_features):
        errors_list.append(f'Number of split categorical series does not '
    ...
    for series in series_in_split_features_list:
        if not series.equals(setup_dataframe[series.name]):
            errors_list.append(f'Split categorical series {series.name} '
                               f'does not match corresponding categorical '
                               f'column in original dataframe.')
    dataframe_in_split_features_list = [x for x in split_features_list if
                                        isinstance(x, pd.DataFrame)]
    if len(dataframe_in_split_features_list) != 1:
        errors_list.append(f'Incorrect number of dataframes returned.')
    elif (len(dataframe_in_split_features_list[0].columns) !=
          len(numeric_features)):
        errors_list.append(f'Number of numeric columns in returned '
        ...
        errors_list.append(f'Returned numeric dataframe does not match '
                           f'original numeric dataframe.')
    assert not errors_list, ('Errors occurred: \n{}'
                             .format('\n'.join(errors_list)))
...
def test_freeze_embedding_layers():
    ...
    layer_out = [layer.trainable for layer in model.layers if
                 type(layer).__name__ == "Embedding"]
...
def test_ffnnm_construct_embedding_network(setup_ffnnm_dataframe,
    ...
    subset_training_obs = (~data['_validation'] & ~data['_test'] &
                           ~data['_predict_obs'])
    train_obs_lead_lengths = (data[subset_training_obs]['_duration']
                              .value_counts())
    n_intervals = train_obs_lead_lengths[
        train_obs_lead_lengths > config['MIN_SURVIVORS_IN_TRAIN']].index.max()
    categorical_features = ['nonmixed_categorical_var',
...
def test_ffnnm_train(setup_ffnnm_dataframe,
                     setup_config):
