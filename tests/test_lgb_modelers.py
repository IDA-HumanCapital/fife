...
def test_gbtm_init():
    ...
    try:
        ...
        if not vars(modeler):
            errors_list.append('GradientBoostedTreesModeler instantiated '
                               'object does not have attributes.')
    except Exception:
        errors_list.append('GradientBoostedTreesModeler could not '
                           'be instantiated.')
...
def test_gbtm_train(setup_config, setup_dataframe):
    ...
    """
    errors_list = []
    cat_features_list = (setup_dataframe.select_dtypes(include=['object'])
                         .columns.tolist())
    for cat_feature in cat_features_list:
        setup_dataframe[cat_feature] = (setup_dataframe[cat_feature]
                                        .astype('category'))
    setup_dataframe['FILE_DATE'] = pd.Series(
        pd.factorize(setup_dataframe['FILE_DATE'])[0])
    subset_training_obs = (~setup_dataframe['_validation'] &
                           ~setup_dataframe['_test'] &
                           ~setup_dataframe['_predict_obs'])
    training_obs_lead_lengths = setup_dataframe[
        subset_training_obs]['_duration'].value_counts()
    ...
    if len(models_list) != n_intervals:
        errors_list.append('Number of models trained and saved does not '
                           'match number of lead lengths.')
    for index, model in enumerate(models_list):
        if not isinstance(model, lgb.basic.Booster):
            errors_list.append(f'Model #{index + 1} (of {len(models_list)}) '
...
def test_gbtm_predict(setup_config, setup_dataframe):
    ...
    """
    errors_list = []
    reserved_cols = ['_test', '_validation', '_predict_obs',
                     '_duration', '_event_observed', 'SSNSCR']
    cat_features_list = (setup_dataframe.select_dtypes(include=['object'])
                         .columns.tolist())
    for cat_feature in cat_features_list:
        setup_dataframe[cat_feature] = (setup_dataframe[cat_feature]
                                        .astype('category'))
    numeric_features_list = [x for x in setup_dataframe.columns if
                             x not in cat_features_list + reserved_cols]
    setup_dataframe['FILE_DATE'] = pd.Series(
        pd.factorize(setup_dataframe['FILE_DATE'])[0])
    subset_training_obs = (~setup_dataframe['_validation'] &
                           ~setup_dataframe['_test'] &
                           ~setup_dataframe['_predict_obs'])
    subset_validation_obs = (setup_dataframe['_validation'] &
                             ~setup_dataframe['_test'] &
                             ~setup_dataframe['_predict_obs'])
    training_obs_lead_lengths = setup_dataframe[
        subset_training_obs]['_duration'].value_counts()
    ...
    for time_horizon in np.arange(n_intervals):
        train_data = setup_dataframe[
            (setup_dataframe['_duration'] +
             setup_dataframe['_event_observed'] >
             time_horizon) & subset_training_obs]
        train_data = lgb.Dataset(train_data[cat_features_list +
                                            numeric_features_list],
                                 label=train_data['_duration'] > time_horizon)
        validation_data = setup_dataframe[
            (setup_dataframe['_duration'] +
             setup_dataframe['_event_observed'] >
             time_horizon) & subset_validation_obs]
    ...
    predictions_dict = {}
    subset_bool_series = (pd.Series(setup_dataframe['SSNSCR'].mod(2))
                          .astype('bool'))
    predictions_dict['cumulative'] = modeler.predict()
    predictions_dict['cumulative_subset'] = modeler.predict(
        subset=subset_bool_series)
    ...
    for case, predictions in predictions_dict.items():
        if predictions.shape[1] != n_intervals:
            errors_list.append(f'Number of periods in predictions array '
                               f'({case}) does not match number of '
                               f'intervals passed to predict() method.')
        if (predictions < 0).any() or (predictions > 1).any():
            errors_list.append(f'One or more of the predicted survival '
    ...
    if (predictions_dict['cumulative'] > predictions_dict['marginal']).any():
        errors_list.append('Cumulative probability predictions exceed '
                           'marginal probability predictions.')
    if ((predictions_dict['cumulative_subset'] >
         predictions_dict['marginal_subset']).any()):
        errors_list.append('Cumulative subset probability predictions '
                           'exceed marginal subset probability predictions.')
    if ((predictions_dict['cumulative'][:, 0] !=
         predictions_dict['marginal'][:, 0]).any()):
        errors_list.append('Cumulative probability predictions do not '
                           'match marginal probability predictions for '
                           'first time interval.')
    if ((predictions_dict['cumulative_subset'][:, 0] !=
         predictions_dict['marginal_subset'][:, 0]).any()):
        errors_list.append('Cumulative subset probability predictions do '
...
def test_gbtm_set_n_intervals(setup_config, setup_dataframe):
    ...
    if not isinstance(n_intervals, (np.int)):
