...
def main():
    ...
    values = [""]
    if by_feature != None:
        values = list(set(df[by_feature]))

    for feature_val in values:
        checkpoint_time = time()
        results_path = config["RESULTS_PATH"]
        if len(values) > 1:
            data = df[df[by_feature] == feature_val]
            results_path = results_path + "//" + feature_val
        else:
            data = df
        ...
        checkpoint_time = time()
        utils.ensure_folder_existence(f"{results_path}/Intermediate/Models")
        ...
        modeler.save_model(path=f"{results_path}/Intermediate/Models/")
        ...
        if test_intervals > 0:
            ...
            utils.save_output_table(
                individual_predictions, "Survival_Curves", path=results_path
            )
            ...
            utils.save_output_table(
                retention_rates, "Retention_Rates", path=results_path
            )
            ...
            utils.save_plot("Retention_Rates", path=results_path)
